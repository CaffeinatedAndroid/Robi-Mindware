...
//HOST COMMAND SERIAL PINS — PICK THESE
#define HOST_RX 4               //NOTE: SOFTWARE SERIAL RX PIN FOR HOST COMMANDS
#define HOST_TX 5               //NOTE: SOFTWARE SERIAL TX PIN FOR HOST COMMANDS
SoftwareSerial hostSerial(HOST_RX, HOST_TX);
...
void setup()
{
  hostSerial.begin(115200);                 //NOTE: OPEN SOFTWARE SERIAL TO HOST FOR COMMANDS
...
void recvWithEndMarker()
{
  ...
  while (hostSerial.available() > 0 && newData == false)   //NOTE: CHANGED FROM Serial TO hostSerial
  {
    rc = hostSerial.read();           //NOTE: READ COMMAND FROM HOST (CHANGED FROM Serial TO hostSerial)
    if (rc != endMarker)
    {
      if (ndx < numChar - 1)
      {
...
void parseData()
{
  ...
  pos = atoi(strIndx);              //NOTE: CONVERT LAST BYTES TO VALID ANGLE
}
