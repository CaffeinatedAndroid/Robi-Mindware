...
class MainApplication:
    ...
    def StartOpenCV(self):
        if self.opencv_instance is None:
            self.tk.update_idletasks()
            self.opencv_instance = VideoFeed(self.opencv_frame, video_source=1, delay=30, coord_var= self.coord_var)
...
class VideoFeed(): #NOTE OPEN CV MANAGER
    ...
    def __init__(self, parent, video_source=1, delay=30, coord_var= None):
    ...
    def update(self):
        ...
        if ret:
            ...
            if self.coord_var:
                self.coord_var.set(f"Position: X: {cx}, Y: {cy}")
    ...
    def stop(self):
        self.vid.release()
        self.parent.after_cancel(self.parent)
